...
async def handler_addgroup(event):
    ...
    if status != 200: return await event.client.send_message("me", "❌ API Error / User not found.")
...
async def handler_apicheck(event):
    ...
    msg = (f"🕵️‍♂️ <b>INFO (CENSORED)</b>\n"
           ...
           f"🌍 IPs: {', '.join([mask_text(i,3,2) for i in ips])}\n"
...
async def handler_pay(event):
    ...
    args = event.pattern_match.group(1)
    await event.delete()
    if not args: return await event.client.send_message("me", "❌ Usage: `.pay [SYMBOL] [AMOUNT]`")
    parts = args.split()
    if len(parts) < 2: return await event.client.send_message("me", "❌ Format: `.pay [SYMBOL] [AMOUNT]`")
    symbol, amount = parts[0].upper(), parts[1]
    wallet = await db.get_wallet(symbol)
    if not wallet: return await event.client.send_message("me", f"❌ Wallet <b>{symbol}</b> not found.", parse_mode='html')
...
async def handler_payadd(event):
    ...
    args = event.pattern_match.group(1)
    await event.delete()
    if not args: return await event.client.send_message("me", "❌ Usage: `.payadd [SYM] [ADDR] [NET]`")
    parts = args.split()
    if len(parts) < 3: return await event.client.send_message("me", "❌ Missing args.")
    symbol, address, network = parts[0].upper(), parts[1], " ".join(parts[2:])
    if await db.set_wallet(symbol, address, network): await event.client.send_message("me", f"✅ Wallet <b>{symbol}</b> saved!", parse_mode='html')
    else: await event.client.send_message("me", "❌ DB Error.")

async def handler_payedit(event): await handler_payadd(event)

async def handler_paycheck(event):
    if not event.out: return
    args = event.pattern_match.group(1)
    await event.delete()
    if not args: return await event.client.send_message("me", "❌ Usage: `.paycheck [SYM] [ADDR] [NET]`")
    parts = args.split()
    if len(parts) < 2:
        symbol = parts[0].upper()
        w = await db.get_wallet(symbol)
        if w: await event.client.send_message("me", f"🔎 {symbol}\n📍 <code>{w['address']}</code>\n🔗 {w['network']}", parse_mode='html')
        else: await event.client.send_message("me", f"❌ {symbol} not found.")
        return
    symbol, address = parts[0].upper(), parts[1]
    network = " ".join(parts[2:]) if len(parts) > 2 else "Default"
    await generate_qr_message(event, address, network, "PREVIEW", is_preview=True)

async def handler_paylist(event):
    if not event.out: return
    await event.delete()
    wallets = await db.get_all_wallets()
    if not wallets: return await event.client.send_message("me", "📂 Empty.")
    msg = "💰 <b>WALLETS</b>\n"
